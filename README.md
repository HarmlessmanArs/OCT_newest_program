...
2) Write mathematical modules for (crossed text means this task has been done):
   ...
   6) parameters calculation
      ...
      2) v2
      3) v3
3) После загрузки должна быть выбрана активная папка, получаем следующую логику: при сохранении необходимо запомнить, какая папка или виджеты
были выбраны; тогда при загрузке получится сразу определить, что показывать пользователю (какие виджеты)

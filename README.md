...
4) Добить созданение и загрузку проекта (пока встречается такая проблема, что при **загрузке проекта не подгружается состояние
...
1) При загрузке проекта поверх другого проекта с открытыми (существующими окнами) происходит вопрос пользователю: 
уверены ли вы, что хотите удалить окно? Нужно, полагаю, сделать так, что если проект в целом не сохранён, то спрашивает пользователя,
точно ли тот хочет закрыть текущий проект (если проект сохранён, то загружается проект). Если проект загружается, то не надо 
спрашивать у пользователя об удалении окон. Наверное, надо воспользоваться self._force_close из widget_controller.py